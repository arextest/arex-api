...
public class FSTraceLogRepositoryImpl implements FSTraceLogRepository {
  ...
  public List<FSTraceLogDto> queryTraceLogsByWorkspaceId(String workspaceId) {
    ...
    List<FSTraceLogCollection> fsTraceLogCollections = mongoTemplate.find(query,
        FSTraceLogCollection.class);
    return fsTraceLogCollections.stream().map(FSTraceLogMapper.INSTANCE::dtoFromDao)
