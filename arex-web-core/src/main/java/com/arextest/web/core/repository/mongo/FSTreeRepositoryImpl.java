...
public class FSTreeRepositoryImpl implements FSTreeRepository {
  ...
  @Resource
  private DistributedLockProvider<RLock> distributedLockProvider;
  ...
  public FSTreeDto updateFSTree(String workspaceId, UnaryOperator<FSTreeDto> unaryOperator) {
    RLock lock = (RLock) distributedLockProvider.getLock(workspaceId);
    try {
  ...
  public List<FSTreeDto> queryFSTreeByIds(Set<String> ids) {
    ...
    List<FSTreeCollection> daos =
        mongoTemplate.find(Query.query(Criteria.where(DASH_ID).in(objectIds)),
  ...
  public Boolean deleteFSTree(String id) {
    DeleteResult result =
        mongoTemplate.remove(Query.query(Criteria.where(DASH_ID).is(new ObjectId(id))),
            FSTreeCollection.class);
