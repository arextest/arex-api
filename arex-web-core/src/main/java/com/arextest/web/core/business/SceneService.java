...
public class SceneService {
  ...
  public void statisticScenes(CompareResultDto compareResultDto) {
    ...
    synchronized (SceneService.class) {
      ...
      for (Map.Entry<String, Map<String, List<Pair<Integer, String>>>> caseEntry : caseMap.entrySet()) {
        ...
        for (Map.Entry<String, List<Pair<Integer, String>>> caseItemEntry : caseEntry.getValue()
            .entrySet()) {
  ...
  private void setBaseTestMissingScene(
      Map<String, Map<String, List<Pair<Integer, String>>>> caseMap,
      String baseMissing) {
  ...
  /**
   * Return the key (node combination) of the scene according to the node name, without sorting (if
   * the future scene is repeated due to the order, it is recommended to do sorting)
   */
  ...
  /**
   * @return : MutablePair<List<NodeEntity>,String> left: List<NodeEntity> List of nodes to
   * aggregate，for calculating fuzzyPath and path right: string last name of nodeEntity, for
   * calculating scenes
   */
  private MutablePair<List<NodeEntity>, String> getPath(UnmatchedPairEntity entity) {
    ...
    if (entity.getUnmatchedType() == UnmatchedType.LEFT_MISSING) {
      ...
      if (!StringUtils
          .isEmpty(entity.getRightUnmatchedPath().get(entity.getRightUnmatchedPath().size() - 1)
              .getNodeName())) {
        nodeName = entity.getRightUnmatchedPath().get(entity.getRightUnmatchedPath().size() - 1)
            .getNodeName();
      }
      ...
      String nodeName = Strings.EMPTY;
      if (!StringUtils
          .isEmpty(entity.getLeftUnmatchedPath().get(entity.getLeftUnmatchedPath().size() - 1)
              .getNodeName())) {
        nodeName = entity.getLeftUnmatchedPath().get(entity.getLeftUnmatchedPath().size() - 1)
            .getNodeName();
      }
      return new MutablePair<>(
          entity.getLeftUnmatchedPath().subList(0, entity.getLeftUnmatchedPath().size() - 1),
          nodeName);
    }
  }
