...
public class ReportDiffAggStatisticRepositoryImpl implements ReportDiffAggStatisticRepository {
  ...
  public DiffAggDto updateDiffScenes(DiffAggDto dto) {
    ...
    for (Map.Entry<String, Map<String, SceneDetailDto>> diffScene : dto.getDifferences()
        .entrySet()) {
  ...
  @Override
  public List<DifferenceDto> queryDifferences(String planItemId, String categoryName,
      String operationName) {
    List<DifferenceDto> differenceDtos = new ArrayList<>();

    Query query = Query.query(
        Criteria.where(PLAN_ITEM_ID).is(planItemId).and(CATEGORY_NAME).is(categoryName)
            .and(OPERATION_NAME).is(operationName));
    ReportDiffAggStatisticCollection dao = mongoTemplate.findOne(query,
        ReportDiffAggStatisticCollection.class);
  ...
  @Override
  public List<SceneDto> queryScenesByDifference(String planItemId, String categoryName,
      String operationName,
      String diffName) {
    ...
    Query query = Query.query(
        Criteria.where(PLAN_ITEM_ID).is(planItemId).and(CATEGORY_NAME).is(categoryName)
            .and(OPERATION_NAME).is(operationName));
    query.fields().include(DIFFERENCES + DOT + diffName).include(PLAN_ITEM_ID)
        .include(CATEGORY_NAME)
        .include(OPERATION_NAME);
    ReportDiffAggStatisticCollection dao = mongoTemplate.findOne(query,
        ReportDiffAggStatisticCollection.class);
    if (dao == null || dao.getDifferences() == null || !dao.getDifferences()
        .containsKey(diffName)) {
      return scenes;
    }
