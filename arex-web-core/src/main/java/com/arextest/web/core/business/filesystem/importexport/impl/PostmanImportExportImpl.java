...
public class PostmanImportExportImpl implements ImportExport {
  ...
  public boolean importItem(FSTreeDto fsTreeDto, List<String> path, String importString) {
    ...
    if (parent != null) {
      ...
      folder = createRootFolder(collection, fsTreeDto.getId(), parent.getInfoId(),
          parent.getNodeType());
  ...
  private FSNodeDto createRootFolder(Collection collection, String workSpaceId, String parentId,
    ...
    return getFolderNode(workSpaceId, parentId, parentNodeType, folderNode, fsFolderDto,
        collection.getItem());
  }

  private FSNodeDto getFolderNode(String workSpaceId, String parentId, Integer parentNodeType,
      FSNodeDto folderNode,
    ...
    if (CollectionUtils.isNotEmpty(items)) {
      items.forEach(collectionItem -> {
        if (collectionItem.getRequest() == null) {
          ...
          children.add(
              createInterfaceNode(collectionItem, workSpaceId, id, folderNode.getNodeType()));
        }
      });
    }
  ...
  private FSNodeDto createInterfaceNode(CollectionItem item, String workSpaceId, String parentId,
    ...
    FSInterfaceDto fsInterfaceDto =
        createFSInterfaceAndCaseBaseDto(itemRequest, workSpaceId, parentId, parentNodeType,
            new FSInterfaceDto());
