...
public class ManualReportCaseRepositoryImpl implements ManualReportCaseRepository {
    ...
    public List<ManualReportCaseDto> initManualReportCases(List<ManualReportCaseDto> caseDtos) {
        List<ManualReportCaseCollection> daos = ManualReportCaseMapper.INSTANCE.daoFromDtoList(caseDtos);
        Collection<ManualReportCaseCollection> results = mongoTemplate.insertAll(daos);
        return results.stream().map(ManualReportCaseMapper.INSTANCE::dtoFromDao).collect(Collectors.toList());
    }
    @Override
    public List<ManualReportCaseDto> queryManualReportCases(List<String> ids) {
        Collection<ObjectId> objectIds = ids.stream().map(id -> new ObjectId(id)).collect(Collectors.toSet());
        Query query = Query.query(Criteria.where(DASH_ID).in(objectIds));
        List<ManualReportCaseCollection> daos = mongoTemplate.find(query, ManualReportCaseCollection.class);
        return daos.stream().map(ManualReportCaseMapper.INSTANCE::dtoFromDao).collect(Collectors.toList());
    }
    @Override
