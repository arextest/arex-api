...
public class GeminiProvider implements AIProvider {
    ...
    private static List<Content> getBasePrompts() {
        ...
        prompts.add(Content.newBuilder().setRole("user").addParts(Part.newBuilder().setText(AIConstants.CONTEXT_PROMPT))
            .build());
        prompts.add(Content.newBuilder().setRole("model")
            ...
            .build());
        prompts.add(
            Content.newBuilder().setRole("user").addParts(Part.newBuilder().setText(AIConstants.USER_Q_1)).build());
        prompts
            .add(Content.newBuilder().setRole("model").addParts(Part.newBuilder().setText(AIConstants.AI_A_1)).build());
        return prompts;
    }
    ...
    public TestScriptGenRes generateScripts(GenReq genReq) {
        try {
            ...
            return AIConstants.MAPPER.readValue(genRes.getCandidates(0)
                    .getContent()
                    .getParts(0)
                    .getText(),
