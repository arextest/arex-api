...
public enum UnmatchedCategory {
  ...
  MATCHED((byte) 0), VALUE_DIFF((byte) 1), LEFT_MISSING((byte) 2), RIGHT_MISSING((byte) 4), UNKNOWN(
      (byte) 8);

  private final static Map<Byte, UnmatchedCategory> CODE_VALUE_MAP = asMap(
      UnmatchedCategory::getCode);
  @Getter
  ...
  public static UnmatchedCategory computeCategory(CompareResultDto compareResult) {
    switch (compareResult.getDiffResultCode()) {
      ...
      default: {
        ...
        int msgMiss =
            Optional.ofNullable(msgInfo).map(MsgInfoDto::getMsgMiss)
                .orElse(MsgMissingCode.NO_MISSING);
